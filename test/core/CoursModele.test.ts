...
describe("CoursModele", () => {
  let tmpDir: string;

  beforeEach(async () => {
    jest.resetModules();
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coursmodele-"));
  ...
  test("obtenirCoursStockes returns empty array when no courses", async () => {
    const store = require("../../src/core/CoursModele");
    const all = await store.obtenirCoursStockes();
    expect(all).toEqual([]);
  });

  test("obtenirCoursStockes returns empty array when courses is not an array", async () => {
    const store = require("../../src/core/CoursModele");
  ...
  test("ajouterCoursStocke adds a course and avoids duplicates", async () => {
    ...
    let all = await store.obtenirCoursStockes();
    expect(all).toHaveLength(1);
    expect(all[0].idGroupe).toBe("g1");

    // adding same idGroupe again shouldn't duplicate
    await store.ajouterCoursStocke(course);
    all = await store.obtenirCoursStockes();
    expect(all).toHaveLength(1);
  });

  test("obtenirCoursStockesPourEnseignant filters by idEnseignant", async () => {
    const store = require("../../src/core/CoursModele");
    ...
    const tA = await store.obtenirCoursStockesPourEnseignant("tA");
  ...
  test("add second course and it is assigned to the teacher", async () => {
    ...
    const t1 = await store.obtenirCoursStockesPourEnseignant("t1");
    expect(t1).toHaveLength(2);
    expect(t1.map((x: any) => x.idGroupe).sort()).toEqual(["g1", "g2"]);
  });

  test("obtenirCoursStockeParIdGroupe finds existing course or returns undefined", async () => {
    const store = require("../../src/core/CoursModele");
    const course = { idGroupe: "gX", jour: "", heure: "", activite: "Act", mode: "", local: "", idEnseignant: "t" } as any;
    await store.ajouterCoursStocke(course);

    const found = await store.obtenirCoursStockeParIdGroupe("gX");
    const notFound = await store.obtenirCoursStockeParIdGroupe("missing");
