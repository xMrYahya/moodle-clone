...
describe("CoursController.index", () => {
  ...
  test("addCourse!=1 with teacher -> renders createdGroups for that teacher", async () => {
    ...
    const getStoredSpy = jest.spyOn(store, "obtenirCoursStockesPourProf").mockResolvedValueOnce([
      { idGroupe: "g-1" },
      { idGroupe: "g-2" },
    ] as any);
    ...
    expect(res.render).toHaveBeenCalledWith("index", expect.objectContaining({
      showAddCourseModal: false,
      createdGroups: [{ idGroupe: "g-1" }, { idGroupe: "g-2" }],
    }));
  });
  ...
  test("addCourse=1 with teacher but no schedules -> createdGroups returned and groups empty", async () => {
    ...
    const store = require("../../src/core/CoursModele");
    jest.spyOn(store, "obtenirCoursStockesPourProf").mockResolvedValueOnce([{ idGroupe: "g-1" }] as any);
  ...
  test("addCourse=1 with teacher: builds groups (direct, fallback, no-dash) and filters createdGroups", async () => {
    ...
    const courses = [
      { id: 123, titre: "Cours 123" },
      { id: 999, titre: "Cours-XYZ-special" }
    ];
    ...
    const store = require("../../src/core/CoursModele");
    jest.spyOn(store, "obtenirCoursStockesPourProf").mockResolvedValueOnce([{ idGroupe: "g-123" }] as any);
    ...
    const renderArg = (res.render.mock.calls[0][1] as any);
    expect(renderArg.createdGroups).toEqual([{ idGroupe: "g-123" }]);
    expect(renderArg.groups.length).toBe(3);

    const gXYZ = renderArg.groups.find((g: any) => g.idGroupe === "g-XYZ");
    const g777 = renderArg.groups.find((g: any) => g.idGroupe === "g-777");
    const gNoDash = renderArg.groups.find((g: any) => g.idGroupe === "groupNoDash");

    expect(gXYZ).toEqual(expect.objectContaining({
      idGroupe: "g-XYZ",
      idCours: "999",
      titreCours: "Cours-XYZ-special",
      activite: "B",
      mode: "N",
      local: "K",
      idEnseignant: "10"
    ...
    expect(gNoDash).toEqual(expect.objectContaining({
      idGroupe: "groupNoDash",
      idCours: "",
      titreCours: "groupNoDash",
      activite: "C",
      mode: "O",
      local: "J",
      idEnseignant: "10"
  ...
  test("accesSGA lance une erreur -> retourne 500 avec message", async () => {
    jest.resetModules();
    const SgbModule = require("../../src/core/sgbClient");
    jest.spyOn(SgbModule.SgbClient.prototype, "getSchedules").mockRejectedValueOnce(new Error("sgb-fail"));
    const store = require("../../src/core/CoursModele");
    jest.spyOn(store, "obtenirCoursStockesPourProf").mockResolvedValueOnce([] as any);
  ...
  test("erreur sans message -> retourne 500 avec message de repli", async () => {
    jest.resetModules();
    const store = require("../../src/core/CoursModele");
    jest.spyOn(store, "obtenirCoursStockesPourProf").mockRejectedValueOnce(undefined);
